...
import { getTotalCustomers, getTotalProducts, getTotalProfitString, getTotalViewsString } from "../utils/getNumberAsString";
...
export function AdminPage() {
    ...
    const [myEventLog, setMyEventLog] = React.useState<EventLog>({} as EventLog);
    const [myEventLogs, setMyEventLogs] = React.useState<EventLog[]>([] as EventLog[]);

    const [data, setData] = React.useState<Data>({} as Data);
    const { totalProfit, totalProducts, totalCustomers, eventLogs } = data;
    ...
    React.useEffect(() => {
        console.log(myEventLogs);
    }, [myEventLogs]);

    const handleAddEventLog = (eventLog: EventLog) => {
        console.log(myEventLogs)
        const newData = [...myEventLogs];
        newData.unshift(eventLog);
        if(newData.length > 10) newData.pop();

        setMyEventLogs(newData);
    }

    React.useEffect(() => {
        (async () => {
            const res = await axios.get(`${URL}/eventLog/data`);
            const myData: Data = res.data;

            setData(myData);
            setMyEventLogs(myData.eventLogs);
        })();
    }, []);

    React.useEffect(() => {
        const eventSource = new EventSource("http://localhost:4000/eventLog");

        eventSource.addEventListener("message", (event) => {
            let eventData: EventLog;
        ...
        <>
            {
                isLoading == true
                    ...
                    :
                    <>
                        {customerState.customer.isAdmin === false && navigate("/")}
                        <NavBar customerState={customerState} />
                        {/* Row 1 */}
                        <div className="px-4 mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
                            <div className="rounded-sm flex flex-col items-center text-center border border-stroke bg-white py-6 px-7.5 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]">
                                <div className="flex h-[2.875rem] w-[2.875rem] items-center justify-center rounded-full bg-gray-200">
                                ...
                                <div className="mt-4 flex items-end justify-between">
                                    <div>
                                        <h4 className="text-title-md font-bold text-green-400 flex justify-center">
                                            {getTotalViewsString(69) || <Spinner />}
                                        </h4>
                            ...
                            <div className="rounded-sm flex flex-col items-center text-center border border-stroke bg-white py-6 px-7.5 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]">
                                ...
                                <div className="mt-4 flex items-end justify-between">
                                    <div>
                                        <h4 className="text-title-md font-bold text-green-400 flex justify-center">
                                            {getTotalProfitString(totalProfit) || <Spinner />}
                                        </h4>
                            ...
                            <div className="rounded-sm flex flex-col items-center text-center border border-stroke bg-white py-6 px-7.5 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]">
                                ...
                                <div className="mt-4 flex items-end justify-between">
                                    <div>
                                        <h4 className="text-title-md font-bold text-green-400 flex justify-center">
                                            {getTotalProducts(totalProducts) || <Spinner />}
                                        </h4>
                            ...
                            <div className="rounded-sm flex flex-col items-center text-center border border-stroke bg-white py-6 px-7.5 shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px]">
                                ...
                                <div className="mt-4 flex items-end justify-between">
                                    <div>
                                        <h4 className="text-title-md font-bold text-blue-400 flex justify-center">
                                            {getTotalCustomers(totalCustomers) || <Spinner />}
                                        </h4>
                        ...
                        {/* Row 2 */}
                        <div className="mx-auto px-4 w-[900px]">
                            <ul>
                                <li className="font-bold border-2 border-gray-300 mb-1">
                                    <div className="px-2 py-1 flex items-center">
                                        <p className="w-[150px] mr-2">Logged</p>
                                        <p>Message</p>
                                    </div>
                                </li>
                                {myEventLogs?.map((eventLog: EventLog) => {
                                    return (
                                        <EventLogItem key={eventLog._id} eventLog={eventLog} />
                                    )
                                })}
                            </ul>
                        </div>
